...
const Stats = ({ feasibilities }) => {
  ...
  useEffect(() => {
    feasibilities.forEach((feasibility) => {
      feasibilitiesByProduct[feasibility.product] = feasibilitiesByProduct[
        feasibility.product
      ]
        ? feasibilitiesByProduct[feasibility.product] + 1
        : 1;
    });
    let max = -1;
  ...
  return feasibilities.length ? (
    <div className="stats">
      <p className="stat">
        Total number of sites processed:{" "}
        <span>
          {feasibilities.reduce(
            (acc, feasibility) => feasibility.numberOfSites + acc,
            0
          )}
        </span>
      </p>
              ...
              (acc, feasibility) =>
                moment(
                  feasibility?.dateCompleted
                    ? feasibility.dateCompleted
                    : moment()
                ).diff(moment(feasibility.dateReceived), "days") + acc,
              0
      ...
      <p className="stat">
        Most popular product: <span>{max}</span>
      </p>
    </div>
  ) : null;
};
