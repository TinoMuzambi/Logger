...
const Home = () => {
  ...
  const getFeasibilities = async () => {
    ...
    console.log(data);
    setFeasibilities(
      data.sort((a, b) => moment(b.dateReceived) - moment(a.dateReceived))
    );
  };
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/feasibilities${
          editMode.state ? `/${editMode.id}` : ""
        }`,
        {
  ...
  const editHandler = (feasibility) => {
    ...
    setFormData({
      ...feasibility,
      dateReceived: moment(feasibility.dateReceived).format("YYYY-MM-DD HH:mm"),
      dateCompleted: feasibility.dateCompleted
        ? moment(feasibility.dateCompleted).format("YYYY-MM-DD HH:mm")
        : "",
    });
  };
          ...
          <form onSubmit={submitHandler}>
            ...
            <div className="input-group">
              ...
              <input
                type="datetime-local"
                id="dateCompleted"
                min={formData.dateReceived ? formData.dateReceived : ""}
                max={moment().format("YYYY-MM-DD HH:mm")}
            ...
            <div className="input-group">
              <button type="submit">
                {editMode.state ? "Update" : "Create new"} Feasibility
              </button>
            </div>{" "}
          </form>
              ...
              <div className="feasibility" key={key}>
                <div className="info">
                  ...
                  <div className="entry">
                    <h3>Date Received:</h3>
                    <p>
                      {moment(feasibility.dateReceived.replace("Z", "")).format(
                        "DD/MM/YYYY HH:mm"
                      )}
                    </p>
                  </div>
                  <div className="entry">
                    ...
                    <p>
                      {feasibility?.dateCompleted
                        ? moment(
                            feasibility?.dateCompleted.replace("Z", "")
                ...
                <div className="buttons">
                  <button onClick={() => editHandler(feasibility)}>
                    <MdModeEdit fill="white" />
                  </button>
                  <button onClick={() => deleteHandler(feasibility._id)}>
                    <MdDelete fill="white" />
                  </button>
                </div>
              </div>
